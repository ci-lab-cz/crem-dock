...
def make_iteration(dbname, iteration, config, mol_dock_func, priority_func, ntop, nclust, mw, rmsd, rtb, logp, tpsa,
    ...
    with sqlite3.connect(dbname) as conn:
        ...
        if make_docking:
            ...
            with sqlite3.connect(dbname) as conn_dock:
                mols = eadb.select_mols_to_dock(conn_dock, add_sql='AND iteration=(SELECT MAX(iteration) from mols)')
                ...
                for i, (mol_id, res) in enumerate(docking(mols,
                    ...
                    if i % 100 == 0:
                        conn_dock.commit()
                    if res:
                        eadb.update_db(conn_dock, mol_id, res, commit=False)
                conn_dock.commit()
